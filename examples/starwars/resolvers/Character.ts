...
import { EEpisode } from "../schema_gen.js";

export async function friends(character: Character): Promise<Character[]> {
	return (await Promise.all(getFriends(character))).filter(
		(c): c is Character => !!c,
	);
}

export function appearsIn(character: Character): EEpisode[] {
	return character.appearsIn.map((e) =>
		e === 4 ? EEpisode.NEW_HOPE : e === 5 ? EEpisode.EMPIRE : EEpisode.JEDI,
	);
}
