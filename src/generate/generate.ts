...
function renderImports(
	...
	extMode:
		...
		| "keep" = "omit", // keep the extension as-is
): jen.Expr {
				...
				.obj(
					...Array.from(symbols)
						.sort(
							(a, b) =>
								a.localeCompare(b) ||
								importAlias(a).localeCompare(importAlias(b)),
						)
						.map((symbol) =>
							symbol in aliases
								? jen.id(aliases[symbol]).as.id(symbol)
								: jen.id(symbol),
						),
				)
				.from.lit(mod);
...
function renderAstNode(astNode: ASTNode): jen.Expr {
	return jen
		...
		.call(
			jen.literal(
				JSON.stringify(astNode, (key, value) =>
...
export interface GenerateOptions {
	...
	fieldDirectivesInfo?: Awaited<
		ReturnType<typeof import("./resolvers.js").parseTypeResolvers>
	>;
	inputDirectivesInfo?: Awaited<
		ReturnType<typeof import("./resolvers.js").parseTypeResolvers>
	>;
	resolversInfo?: Awaited<ReturnType<typeof import("./resolvers.js").parse>>;
	subscribersInfo?: Awaited<
		ReturnType<typeof import("./resolvers.js").parseTypeResolvers>
	>;
...
class GeneratorContext {
	...
	private _fieldDirectivesInfo: Exclude<
		GenerateOptions["fieldDirectivesInfo"],
		undefined
	>;
	private _inputDirectivesInfo: Exclude<
		GenerateOptions["inputDirectivesInfo"],
		undefined
	>;
	private _resolversInfo: Exclude<GenerateOptions["resolversInfo"], undefined>;
	private _subscribersInfo: Exclude<
		GenerateOptions["subscribersInfo"],
		undefined
	>;
	...
	private _renderGraphQLTypeIdentifier(
		...
		if (type instanceof GraphQLNonNull) {
			return jen.new
				.add(this._gql("GraphQLNonNull"))
				.params(this._renderGraphQLTypeIdentifier(type.ofType));
		}

		if (type instanceof GraphQLList) {
			return jen.new
				.add(this._gql("GraphQLList"))
				.params(this._renderGraphQLTypeIdentifier(type.ofType));
		}
				...
				.map((arg: GraphQLArgument & { defaultValue?: any }) =>
					jen.prop(
						...
						jen.obj(
							...
							...truthify([
								arg.description &&
									jen.prop("description", jen.lit(arg.description)),
								arg.defaultValue &&
									jen.prop("defaultValue", jen.lit(arg.defaultValue)),
								arg.deprecationReason &&
									jen.prop("deprecationReason", jen.lit(arg.deprecationReason)),
							]),
						),
					),
				),
	...
	private _renderValueNode(node: ValueNode): jen.Expr {
		switch (node.kind) {
			...
			case Kind.OBJECT:
				return jen.obj(
					...node.fields.map((f) =>
						jen.prop(f.name.value, this._renderValueNode(f.value)),
					),
				);
		}
	...
	private _renderArgumentNodes(
		nodes: readonly ConstArgumentNode[] | undefined,
	): jen.Expr {
		return jen.obj(
			...Array.from(nodes ?? [], (arg) =>
				jen.prop(arg.name.value, this._renderValueNode(arg.value)),
			),
		);
	}
	...
	private _renderFieldObject(
		...
		f: (GraphQLInputField | GraphQLField<unknown, unknown>) &
			Partial<Pick<GraphQLField<unknown, unknown>, "args">> & {
				// biome-ignore lint/suspicious/noExplicitAny: <explanation>
				defaultValue?: any;
			},
		inputFieldDirectives: Record<string, Record<string, string[]>>,
		...
		if (!noresolver) {
			...
			if (type instanceof GraphQLObjectType && !resolver) {
				...
				for (const iface of type.getInterfaces()) {
					resolver = this._resolversInfo[iface.name]?.[f.name];
					if (resolver) {
						break;
					}
				}
			...
			if (f.astNode?.directives?.length) {
				...
				for (const directive of Array.from(f.astNode.directives).reverse()) {
					...
					if (!resolveFn) {
						resolveFn = jen
							.arrow(jen.id("source").op(":").any)
							.id("source")
							.dot(f.name);
					}

					resolveFn = renderSymbolReference(
						this._pkgs,
						ref,
						`apply${pascalCase(directive.name.value)}FieldDirective`,
					).call(
						resolveFn,
			...
			if (resolveFn) {
				...
				const argsWithDirectives = truthify(
					f.args?.map((arg) =>
						this._findInputDirectiveInfoFromArgOrField(
							arg,
						...
						.parens(
							jen
								.params(
									jen.prop(
										"next",
										this._gql("GraphQLFieldResolver").types(jen.any, jen.any),
									),
								)
								.op(":")
								...
								.types(
									jen.any,
									jen.any,
									jen.id(type.name).dot(pascalCase(`${f.name}_args`)),
								)
								.op("=>")
								.arrow(
									jen.id("source"),
								...
								.call(
									...
									this._renderInputFieldsWithDirectives(
										jen.id("args"),
										argsWithDirectives,
										argsWithDirectives.length < (f.args?.length ?? 0),
									),
									jen.id("context"),
									jen.id("info"),
								),
		...
		const subscriber =
			type.name === "Subscription" ? this._subscribersInfo[f.name] : undefined;
		...
		return jen.obj(
			...
			...truthify([
				...
				f.deprecationReason &&
					jen.prop("deprecationReason", jen.lit(f.deprecationReason)),
				...
				subscriber &&
					jen.prop(
						"subscribe",
						jen.parens(renderSymbolReference(this._pkgs, subscriber)).as.any,
					),
	...
	private _renderOptionalGraphQLTypeType(type: GraphQLType): jen.Expr {
		if (type instanceof GraphQLNonNull) {
			return this._renderGraphQLTypeType(type);
		}

		return jen.union(this._renderGraphQLTypeType(type), jen.undefined);
	}
	...
	private _findScalarType(type: GraphQLScalarType): string | undefined {
		...
		if (!returnType) {
			console.log(
				`WARNING: Could not determine scalar ${type.name} type from parseValue or parseLiteral`,
			);
		}

		this._scalarTypeTypeCache[type.name] = returnType;

		return returnType;
	}

	private _renderGraphQLTypeType(type: GraphQLType): jen.Expr {
		if (type instanceof GraphQLNonNull) {
		...
		if (type instanceof GraphQLList) {
			return jen
				.id("Array")
				.types(this._renderOptionalGraphQLTypeType(type.ofType));
		}
	...
	private _renderTypeInterface(
		...
		return jen.interface.add(this._renderGraphQLTypeType(type)).block(
			...Object.values(type.getFields())
				.sort(compareName)
				.map((field) =>
					jen.prop(
	...
	private _renderGraphQLObjectTypeDefinition(
		...
		return jen.new
			.add(this._gql("GraphQLObjectType"))
			.call(
				jen.obj(
					jen.prop("name", jen.lit(type.name)),
					jen.prop(
						"fields",
						this._renderFieldsThunk(
							type,
							inputFieldDirectives,
							includeAstNodes,
							!this._includeSubscriptionFieldResolvers &&
					...
					...truthify([
						type.description &&
							jen.prop("description", jen.lit(type.description)),
						...
						includeAstNodes &&
							type.astNode &&
							jen.prop("astNode", renderAstNode(type.astNode)),
					]),
				),
			)
	...
	private _renderGraphQLObjectTypeTypes(type: GraphQLObjectType): jen.Expr[] {
								...
								jen.export.interface.id(pascalCase(`${field.name}_args`)).block(
									...Array.from(field.args)
										...
										.map((arg) =>
											jen.prop(
												arg.name,
												this._renderOptionalGraphQLTypeType(arg.type),
											),
										),
								),
							),
	...
	private _renderGraphQLScalarTypeDefinition(
		...
		return jen.new
			.add(this._gql("GraphQLScalarType"))
			.call(
				jen.obj(
					jen.prop("name", jen.lit(type.name)),
					...truthify([
						...
						jen.prop(
							"serialize",
							s?.serializeFunc
								? renderSymbolReference(this._pkgs, s.serializeFunc)
								: jen.trivia("stub").arrow(jen.id("v")).id("v"),
						),
						s?.parseValueFunc &&
							jen.prop(
								"parseValue",
								renderSymbolReference(this._pkgs, s.parseValueFunc),
							),
						s?.parseLiteralFunc &&
							jen.prop(
								"parseLiteral",
								renderSymbolReference(this._pkgs, s.parseLiteralFunc),
							),
					]),
				),
			);
	...
	private _renderTypeResolverAndCastFunctions(
		...
		return [
			jen
				.arrow(jen.prop("v", jen.any))
				.block(
					jen
						.switch(
							jen.id("v").computed(jen.id(`${prefix}Symbol_${type.name}`)),
						)
						.block(
			...
			[
				jen.const
					.id(`${prefix}Symbol_${type.name}`)
					.op("=")
					.id("Symbol")
					.call(jen.lit(`${type.name} type`)),
				...sortedSubTypes.map((subType) =>
					jen.const
						.id(`${prefix}TypeSymbol_${subType.name}_${type.name}`)
						.op("=")
						.id("Symbol")
						.call(jen.lit(`${subType.name} ${type.name}`)),
				),
				...sortedSubTypes.map((subType) =>
					jen.export.const
						.id(`as${subType.name}${type.name}`)
						.op("=")
						.types(jen.id("T"))
						.params(jen.prop("v", jen.any))
						.op(":")
						.intersect(
							jen.id("T"),
							jen.obj(
	...
	private _renderGraphQLInterfaceTypeDefinition(
		...
		return jen.new
			.add(this._gql("GraphQLInterfaceType"))
			.call(
				jen.obj(
					jen.prop("name", jen.lit(type.name)),
					jen.prop(
	...
	private _renderGraphQLInterfaceTypeTypes(
		type: GraphQLInterfaceType,
		{ implementors: { implementors } }: TypeRenderContext,
	): jen.Expr[] {
		const fieldsWithArgs = Object.values(type.getFields())
			.filter((field) => field.args.length)
			.sort(compareName);
		return [
			...
			jen.export.namespace.id(type.name).block(
				...implementors.map((implementor) =>
					jen.export.const
						.id(`as${pascalCase(implementor.name)}`)
						.op("=")
						.types(jen.id("T"))
						.params(jen.prop("v", jen.id("T")))
						.op("=>")
						.id("setTypename")
						.call(jen.id("v"), jen.lit(implementor.name)),
				),
	...
	private _renderGraphQLUnionTypeDefinition(type: GraphQLUnionType): jen.Expr {
		return jen.new
			.add(this._gql("GraphQLUnionType"))
			.call(
				jen.obj(
					jen.prop("name", jen.lit(type.name)),
					jen.prop(
						...
						jen
							...
							.array(
								...type
									.getTypes()
									.map((type) => this._renderGraphQLTypeIdentifier(type)),
							),
					),
					...truthify([
						type.description &&
	...
	private _renderGraphQLUnionTypeTypes(type: GraphQLUnionType): jen.Expr[] {
		return [
			jen.export.type
				.add(this._renderGraphQLTypeType(type))
				.op("=")
				.union(
					...Array.from(type.getTypes())
						.sort(compareName)
						.map(this._renderGraphQLTypeType, this),
				),
			jen.export.namespace.id(type.name).block(
				...type.getTypes().map((type) =>
					jen.export.const
						.id(`as${pascalCase(type.name)}`)
						.op("=")
						.types(jen.id("T"))
						.params(jen.prop("v", jen.id("T")))
						.op("=>")
						.id("setTypename")
						.call(jen.id("v"), jen.lit(type.name)),
				),
			),
		];
	}
	...
	private _findInputDirectiveInfoFromArgOrField(
		...
		if (
			inputFieldDirectives[fieldTypeName] &&
			Object.keys(inputFieldDirectives[fieldTypeName]).length
		) {
			transform = {
	...
	private _renderInputFieldsWithDirectives(
		valueExpr: jen.Expr,
		fieldsWithDirectives: Exclude<
			ReturnType<typeof this._findInputDirectiveInfoFromArgOrField>,
			undefined
		>[],
		spread: boolean,
	) {
		return jen.obj(
			...truthify([spread && jen.spread(valueExpr)]),
			...fieldsWithDirectives.map(({ field, directives, transform }) => {
				...
				if (transform) {
					transformedExpr = renderSymbolReference(this._pkgs, transform).call(
						transformedExpr,
					);
				}
	...
	private _renderGraphQLInputObjectTypeDefinition(
		...
		return jen.new
			.add(this._gql("GraphQLInputObjectType"))
			.call(
				jen.obj(
					jen.prop("name", jen.lit(type.name)),
					jen.prop(
	...
	private _renderGraphQLInputObjectTypeTypes(
		...
		const fieldsWithDirectives = truthify(
			Object.values(type.getFields()).map((f) =>
				this._findInputDirectiveInfoFromArgOrField(f, inputFieldDirectives),
			),
		);
			...
			jen.export.interface.add(this._renderGraphQLTypeType(type)).block(
				...Object.values(type.getFields())
					...
					.map((field) =>
						jen.prop(
							field.name,
							this._renderOptionalGraphQLTypeType(field.type),
						),
					),
			),
			...truthify([
				fieldsWithDirectives.length &&
					jen.const
						.id(`transform${this._deriveGraphQLTypeName(type)}`)
						.op("=")
						.types(jen.id("T").extends.union(jen.id(type.name), jen.undefined))
						.params(jen.prop("v", jen.id("T")))
						.op(":")
						.id("T")
						.op("=>")
							...
							...sortedValues.map((e) =>
								jen.prop(
									...
									jen.obj(
										...
										...truthify([
											e.description &&
												jen.prop("description", jen.lit(e.description)),
											e.deprecationReason &&
												jen.prop(
													"deprecationReason",
													jen.lit(e.deprecationReason),
												),
										]),
									),
								),
	...
	private _renderGraphQLEnumTypeTypes(type: GraphQLEnumType): jen.Expr[] {
		...
		return [
			jen.export.enum
				.add(this._renderGraphQLTypeType(type))
				.obj(...sortedValues.map(({ name }) => jen.id(name).op("=").lit(name))),
			jen.export
				.function(
				...
				.op(":")
				.id("v")
				.is.add(this._renderGraphQLTypeType(type))
				.block(
	...
	private _getRenderDefintionFnForType(
		type: GraphQLType,
	): (ctx: TypeRenderContext) => jen.Expr {
		if (type instanceof GraphQLObjectType) {
	...
	private _getRenderTypesFnForType(
		type: GraphQLType,
	): (ctx: TypeRenderContext) => jen.Expr[] {
		if (type instanceof GraphQLObjectType) {
	...
	private _renderGraphQLDirectiveDefinition(
		directive: GraphQLDirective,
		includeAstNodes?: boolean,
	): jen.Expr {
		return jen.const
			.add(this._renderGraphQLTypeIdentifier(directive))
			.op("=")
			.new.add(this._gql("GraphQLDirective"))
			.call(
				jen.obj(
					...
					jen.prop(
						"locations",
						jen.array(
							...Array.from(directive.locations)
								.sort()
								.map((location) =>
					...
					...truthify([
						directive.description &&
							jen.prop("description", jen.lit(directive.description)),
						directive.isRepeatable && jen.prop("isRepeatable", jen.true),
						includeAstNodes &&
							directive.astNode &&
							jen.prop("astNode", renderAstNode(directive.astNode)),
					]),
				),
			);
	}

	private _renderGraphQLDirectiveTypes(
		directive: GraphQLDirective,
	): jen.Expr[] {
		return truthify([
			directive.args.length &&
				jen.export.interface
					.id(pascalCase(`${directive.name}_directive_args`))
					.block(
	...
	public schemaExprs(
		...
		[types, interfaces, inputFieldDirectives, includeAstNodes]: ReturnType<
			(typeof GeneratorContext)["prepareInterfacesAndInputFieldDirectives"]
		>,
	): jen.Expr[] {
		...
		return [
			...[
				...
				...types
					.map((type): [string, jen.Expr] => [
						type.name,
						this._renderGraphQLTypeDefinitions(
							type,
							interfaces,
							inputFieldDirectives,
							includeAstNodes,
				...
				...schema
					...
					.map((directive): [string, jen.Expr] => [
						directive.name,
						this._renderGraphQLDirectiveDefinition(directive),
					])
					.sort(compareEntryKey),
			].map(([, expr]) => expr),

			...Array.from(PRIMITIVE_TYPES).flatMap((type) => {
				...
				return [
					jen.statements(
						...([
							info.serializeFunc &&
								this._gql(`GraphQL${type}`)
									.dot("serialize")
									.op("=")
									.parens(
										renderSymbolReference(this._pkgs, info.serializeFunc).as
											.any,
									),
							info.parseValueFunc &&
								this._gql(`GraphQL${type}`)
									...
									.op("=")
									.parens(
										renderSymbolReference(this._pkgs, info.parseValueFunc).as
											.any,
									),
							info.parseLiteralFunc &&
								this._gql(`GraphQL${type}`)
									...
									.op("=")
									.parens(
										renderSymbolReference(this._pkgs, info.parseLiteralFunc).as
											.any,
									),
			...
			jen.const
				.id("schema")
				.op("=")
				.new.add(this._gql("GraphQLSchema"))
				.call(
					jen.obj(
						...(queryType
							? [
						...
						jen.prop(
							"types",
							jen.array(
								...types.map((type) => this._renderGraphQLTypeIdentifier(type)),
							),
						),
						jen.prop(
							"directives",
	...
	public typesExprs(
		schema: GraphQLSchema,
		[types, interfaces, inputFieldDirectives]: ReturnType<
			(typeof GeneratorContext)["prepareInterfacesAndInputFieldDirectives"]
		>,
	): jen.Expr[] {
		return [
			[
				...
				jen.const
					...
					.op("=")
					.types(jen.id("T"))
					.params(jen.id("v").op(":").id("T"), jen.id("t").op(":").string)
					.op(":")
					.id("T")
					.op("=>")
					.block(
						...
						jen.return(
							jen
								...
								.call(
									jen.id("v"),
									jen.obj(jen.prop(jen.id("__typename"), jen.id("t"))),
								),
						),
					),
			] as const,
			...
			...schema
				...
				.map((directive): [string, jen.Expr] => [
					directive.name,
					jen.statements(...this._renderGraphQLDirectiveTypes(directive)),
				])
				.sort(compareEntryKey),
		].map(([, expr]) => expr);
	}
	...
	public static prepareInterfacesAndInputFieldDirectives(
		...
		if (options.fieldDirectivesInfo) {
			...
			for (const directive of schema.getDirectives()) {
				if (
					...
					directive.locations.every(
						(loc) => loc !== DirectiveLocation.FIELD_DEFINITION,
					) ||
				...
				console.log(
					`WARNING: missing directive function for ${JSON.stringify(
						directive.name,
					)}`,
				);
			}
		...
		const types = Object.entries(schema.getTypeMap())
			.sort(compareEntryKey)
			.filter(
				([, type]) => !type.name.startsWith("__") && !isPrimitiveType(type),
			)
			.map(([, type]) => type);
		...
		if (options.inputDirectivesInfo) {
			...
			for (const type of types) {
				...
				for (const field of Object.values(type.getFields())) {
					...
					for (const directive of directives) {
						...
						if (!inputFieldDirectives[type.name]) {
							inputFieldDirectives[type.name] = {};
						}

						const fieldDirectives = inputFieldDirectives[type.name];
						if (!fieldDirectives[field.name]) {
							fieldDirectives[field.name] = [];
						}

						fieldDirectives[field.name].push(name);
					}
				}
			}
			...
			for (let changes = 1; changes; changes = 0) {
				...
				for (const [
					...
					fieldName,
					objectTypeName,
				] of typesWithObjectFields) {
					console.log(
						`checking ${typeName}.${fieldName} for ${objectTypeName}`,
					);
					if (inputFieldDirectives[objectTypeName]) {
						inputFieldDirectives[typeName] = {
							...inputFieldDirectives[typeName],
							[fieldName]: [],
						};
						changes += 1;
					} else {
						newTypesWithObjectFields.push([
							typeName,
							fieldName,
							objectTypeName,
						]);
...
export function createGenerator(
	...
	const args = GeneratorContext.prepareInterfacesAndInputFieldDirectives(
		schema,
		options,
	);
	return {
		generateSchema: (outfile: string) =>
			new GeneratorContext(options).renderFile(
				outfile,
				function (this: GeneratorContext) {
					return this.schemaExprs(schema, args);
				},
			),
		generateTypes: (outfile: string) =>
			new GeneratorContext(options).renderFile(
				outfile,
				function (this: GeneratorContext) {
