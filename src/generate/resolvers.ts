...
export async function parseTypeResolvers(typeName: string, glob: string | string[]): Promise<Record<string, SymbolReference>> {
...
export async function parse(dirpath: string): Promise<Record<string, Record<string, SymbolReference>>> {
  ...
  for await (const name of await fs.readdir(dirpath)) {
    ...
    if (entry.isDirectory()) {
      out[name] = await parseTypeResolvers(name, file + "/*.ts");
    } else if (entry.isFile() && name.endsWith(".ts")) {
      out[name.slice(0, -3)] = await parseTypeResolvers(name.slice(0, -3), file);
    }
  }
