...
export class RequiredPackages {
	...
	constructor(public readonly requires: Record<string, Set<string>> = {}) {
	...
	public addRequires(pkg: string, name: string, alias?: string) {
		...
		if (
			(symbol in this.symbols || keywords.has(symbol)) &&
			this.symbols[symbol] !== pkg
		) {
			for (
				let i = 1;
				// biome-ignore lint/suspicious/noAssignInExpressions: it works don't fix it
