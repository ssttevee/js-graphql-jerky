...
function isListValueNodeEqual(
  a: ListValueNode,
  b: ListValueNode,
): boolean {
  return zip(a.values as ValueNode[], b.values as ValueNode[]).every(([a, b]) => isValueNodeEqual(a, b));
}
...
function isObjectValueNodeEqual(
  ...
  return zip(sortNamedNodes(a.fields), sortNamedNodes(b.fields))
    .every(([a, b]) => a.name.value === b.name.value && isValueNodeEqual(a.value, b.value));
}
...
function isArgumentNodesEqual(
  ...
  return zip(sortNamedNodes(a), sortNamedNodes(b)).every(
    ([a, b]) => a.name.value === b.name.value && isValueNodeEqual(a.value, b.value),
  );
}

function isTypeNodeEqual(
  a: TypeNode,
  b: TypeNode,
): boolean {
  if (a.kind !== b.kind) {
...
function isInputValueDefinitionNodesEqual(
  ...
  return zip(sortNamedNodes(a), sortNamedNodes(b)).every(
    ([a, b]) =>
      a.name.value === b.name.value && isTypeNodeEqual(a.type, b.type) &&
  ...
  if (b) {
    for (const d of b) {
      if (
        ...
        !isArgumentNodesEqual(
          directives[d.name.value].arguments,
          d.arguments,
        )
      ) {
        throw setTraceToSchema(
...
export function mergeDocumentNodes(
  ...
  for (const documentNode of documentNodes) {
    for (const definition of documentNode.definitions) {
      ...
      if (
        prevDefinition.description && definition.description &&
      ...
      switch (definition.kind) {
        ...
        case Kind.INTERFACE_TYPE_DEFINITION:
          definitions[definition.name.value] = mergeInterfaceTypeDefinitionNodes(
        ...
        case Kind.INPUT_OBJECT_TYPE_DEFINITION:
          definitions[definition.name.value] = mergeInputObjectTypeDefinitionNodes(
