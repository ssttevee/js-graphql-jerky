...
function locLineColumn(decl: ts.Declaration, source: ts.SourceFile): [number, number] {
...
function refFromImportTypeNode(node: ts.ImportTypeNode, source: ts.SourceFile): SymbolReference | undefined {
  if (node.isTypeOf) {
  ...
  if (!ts.isLiteralTypeNode(node.argument) || !ts.isStringLiteral(node.argument.literal)) {
    ...
    console.log(`WARNING: grammar error at ${source.fileName}:${line + 1}:${character + 1}`);
    return;
  }
    ...
    if (ts.isImportDeclaration(statement)) {
      ...
      if (statement.importClause.name) {
        ...
        if (statement.importClause.name.text === typeName) {
          return { module: statement.importClause.name.text, symbol: 'default', alias: typeName };
        }
      }
      ...
      for (const specifier of statement.importClause.namedBindings.elements ?? []) {
        if (specifier.name.text === typeName) {
          return { module: statement.moduleSpecifier.text, symbol: specifier.propertyName?.text ?? typeName, alias: typeName };
        }
      }
    } else if (ts.isTypeAliasDeclaration(statement) || ts.isInterfaceDeclaration(statement) || ts.isEnumDeclaration(statement)) {
      if (statement.name.text === typeName) {
        if (!statement.modifiers?.some((mod) => mod.kind === ts.SyntaxKind.ExportKeyword)) {
          console.log(statement);
          console.log(`WARNING: found type ${typeName} in ${source.fileName}:${locLineColumn(statement, source).join(':')} but it is not exported`);
          continue;
        }
...
function refFromTypeNode(typeNode: ts.TypeNode, source: ts.SourceFile): SymbolReference | undefined {
  if (ts.isImportTypeNode(typeNode)) {
    return refFromImportTypeNode(typeNode, source);
  } else if (ts.isTokenKind(typeNode.kind)) {
    ...
    console.warn("WARNING: unexpected type node: " + ts.SyntaxKind[typeNode.kind]);
  }
}
...
function reconcileScalarType(
  ...
  for (const typeNode of typeDecls) {
    ...
    let newType = refFromTypeNode(typeNode, source);
    if (type && newType) {
      if (
        ...
        throw new Error(`mismatched types do not match for scalar ${scalarName}: ${type.symbol} !== ${newType.symbol}`);
      }
          ...
          switch (prop.name.text) {
            ...
            case "parseLiteral":
              if (!ts.isFunctionExpression(prop.initializer) && !ts.isArrowFunction(prop.initializer)) {
                console.log(
                  `WARNING: expected scalar property ${scalarName}.${prop.name.text} to be a function, but got ${ts.SyntaxKind[prop.initializer.kind]
                  }`,
                );
                return [];
              }
              ...
              return [[
                `${prop.name.text as "serialize" | "parseValue" | "parseLiteral"}Func`,
                {
                  module: module,
                  symbol: scalarName,
...
export async function parse(path: string): Promise<Record<string, ScalarInfo>> {
  ...
  for (const name of new Set([...Object.keys(scalarFnsInfos), ...Object.keys(scalarTypeNodes)])) {
